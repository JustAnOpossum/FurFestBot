...
const random = new RandomOrg({ apiKey: fs.readFileSync(path.join(__dirname, "../private/randomkey.txt"), 'utf8').toString() });

exports.pickImage = function () {
return new Promise(function(res, rej){
  let randNum
  random.generateIntegers({ min: 1, max: 1181, n: 1 })
  .then(function(result) {
    randNum = result.random.data[0]
    findSize()
  });
  function findSize() {
  gm(num.pics[randNum]).size(function(err, num){if(!err){writeImage(num.width, num.height)}else{console.log(err)}})
}
  ...
  .fontSize(width/5)
  .write(path.join(__dirname, "../Countdown/"+day.toString()+'.jpg'), function(err){if(!err){console.log('Done')
